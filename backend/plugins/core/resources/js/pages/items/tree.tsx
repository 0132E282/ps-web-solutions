...
const TreePage = () => {
    ...
    const { current: resourceName, views, crudRoutes } = useModule();
    ...
    useEffect(() => {
        if (isEdit && selectedId && resourceName) {
             const fieldNames = [
        ...
        <div className="flex flex-col lg:flex-row gap-6">
            <div className="w-full lg:w-[400px]">
                <TreeSidebar
                    ...
                    onDelete={(id) => {
                        if (resourceName) {
                            dispatch(deleteResourceRequest({ resource: resourceName, id }));
                            if (selectedId === id) setSelectedId(null);
                        }
                    }}
                    onDuplicate={(id) => {
                        if (resourceName) {
                            dispatch(duplicateResourceRequest({ resource: resourceName, id }));
                        }
                    }}
                    onMove={(id, parentId) => {
                        if (resourceName) {
                    ...
                    onBulkAction={(action, ids) => {
                        if (resourceName && action === 'delete') {
                            dispatch(bulkDeleteResourceRequest({ resource: resourceName, ids }));
                            setSelectedIds([]);
                            if (ids.includes(selectedId as any)) setSelectedId(null);
                        }
                    }}
                />
            </div>
            <div className="flex-1">
                <Card className="p-6">
                    <FormPages
                        key={selectedId || 'new'}
                        defaultValues={(reduxItem as any) || {}}
                        onSubmit={(data) => {
                        ...
                        onViewModeChange={(val) => {
                            if (val === 'table' && crudRoutes.index) {
                                window.location.href = route(crudRoutes.index);
                            }
                        }}
                    >
                        <div className="grid grid-cols-1 gap-6">
                            <div className="space-y-6">
                                ...
                                {sidebarSections.length > 0 && sidebarSections.map((section: any, index: number) => (
