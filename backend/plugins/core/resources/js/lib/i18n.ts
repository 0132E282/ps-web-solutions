...
const getInitialData = (): any => {
    ...
    if ((window as any).__INERTIA_INITIAL_DATA__) return (window as any).__INERTIA_INITIAL_DATA__;
...
const getAvailableNamespaces = (translations: any, availableLocales: string[]): string[] => {
...
const getTranslationsFromInertia = (): TranslationsData => {
    const translations = getInitialData()?.props?.translations || {};
    ...
    availableLocales.forEach((locale) => {
        const localeData = translations[locale];
        const localeResult: Record<string, any> = {};
...
const isValidLocale = (locale: string | null | undefined, availableLocales: string[]): boolean => {
    return locale !== null && locale !== undefined && (availableLocales.length === 0 || availableLocales.includes(locale));
};
...
const getInitialLocale = (): string => {
    ...
    if (typeof window !== 'undefined') {
        ...
        if (isValidLocale(queryLocale, availableLocales)) return queryLocale!;
    }
    ...
    if (typeof document !== 'undefined') {
        ...
        if (isValidLocale(cookieLocale, availableLocales)) return cookieLocale!;
            }
    
    return availableLocales[0] || (typeof navigator !== 'undefined' ? navigator.language?.split('-')[0] : null) || 'en';
};
...
const availableNamespaces = getAvailableNamespacesFromResources();
const defaultNamespace = availableNamespaces.includes('common') ? 'common' : availableNamespaces[0] || 'common';
const getFallbackLanguage = (): string => availableLocales[0] || 'en';

export const tt = (key: string, options?: any): string => {
    try {
...
i18n.use(LanguageDetector)
    ...
    .init({
        ...
        supportedLngs: availableLocales.length > 0 ? availableLocales : undefined,
...
if (typeof window !== 'undefined') {
    const addResourceBundle = (locale: string, namespace: string, data: any) => {
    ...
    if (typeof window !== 'undefined' && (window as any).__CLEANUP_INTERVALS) {
        (window as any).__CLEANUP_INTERVALS.push(() => clearInterval(urlCheckInterval));
    }
